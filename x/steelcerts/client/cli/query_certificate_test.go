...
import (
	...
	"github.com/sapcis/steelcerts/testutil/network"
	"github.com/sapcis/steelcerts/x/steelcerts/client/cli"
...
func networkWithCertificateObjects(t *testing.T, n int) (*network.Network, []types.Certificate) {
	...
	for i := 0; i < n; i++ {
		state.CertificateList = append(state.CertificateList, types.Certificate{
...
func TestListCertificate(t *testing.T) {
	...
	t.Run("ByOffset", func(t *testing.T) {
		...
		for i := 0; i < len(objs); i += step {
			...
			require.Subset(t, objs, resp.Certificate)
		}
	})
